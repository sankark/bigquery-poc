$(document).ready(function () {
    ...
    theme = $.trim(theme.replace("container", "").replace("container-fixed", ""));
    if (theme == '')
        theme = 'default';
...
function local_get() {
    if (typeof(Storage) !== "undefined") {
        ...
        if (sessionStorage.siteContainer != '.container') {
            if ($('.container').hasClass('container-fixed'))
                $('.container').attr('class', sessionStorage.siteContainer).addClass('container-fixed');
            else
                $('.container').attr('class', sessionStorage.siteContainer);
        }
    }
}
